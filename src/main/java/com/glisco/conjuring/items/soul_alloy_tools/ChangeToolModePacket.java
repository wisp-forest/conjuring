...
public class ChangeToolModePacket {
    ...
    public static void onPacket(MinecraftServer server, ServerPlayerEntity player, ServerPlayNetworkHandler handler, PacketByteBuf buffer, PacketSender sender) {
        server.execute(() -> {
            ...
            player.playSound(SoulAlloyTool.isSecondaryEnabled(player.getMainHandStack()) ? SoundEvents.BLOCK_RESPAWN_ANCHOR_SET_SPAWN : SoundEvents.BLOCK_RESPAWN_ANCHOR_DEPLETE.value(), SoundCategory.PLAYERS, 1, 1.5f + player.getWorld().random.nextFloat() * 0.5f);
        });
    }
