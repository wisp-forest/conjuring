...
public abstract class ModifiedAbstractSpawner extends AbstractSpawner {
    ...
    public void tick() {
        if (!this.isPlayerInRange()|| !active) {
            ...
            if (!(world instanceof ServerWorld)) {
                ...
                //These particles are different to reflect the custom spawner version
                world.addParticle(ParticleTypes.ENCHANTED_HIT, d3, d4, d5, 0.0D, 0.0D, 0.0D);
                ...
                for(int i = 0; i < this.spawnCount; ++i) {
                    ...
                    if (world.hasNoCollisions(optional.get().getBoundingBoxWithSizeApplied(d0, d1, d2))) {
                        ...
                        if (EntitySpawnPlacementRegistry.canSpawnEntity(optional.get(), serverworld, SpawnReason.SPAWNER, new BlockPos(d0, d1, d2), world.getRandom())) {
                            ...
                            world.playEvent(9004, blockpos, 0);
                            if (entity instanceof MobEntity) {
    ...
    public void setRequiredPlayerRange(int requiredPlayerRange) {
        this.activatingRangeFromPlayer = requiredPlayerRange;
    }
    ...
    public void setSpawnPotentials(List<WeightedSpawnerEntity> spawnPotentials) {
        this.potentialSpawns.clear();
